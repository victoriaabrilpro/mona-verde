...
const PrivacyPolicyModal: React.FC<PrivacyPolicyModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const fetchPolicy = async () => {
      try {
        const data = await client.fetch<PrivacyPolicy>(queries.privacyPolicy);
        setPolicyData(data);
        setLoading(false);
      } catch (error) {
          ...
          >
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-white/80 text-sm font-colfax-regular">
                    {policyData?.subtitle || 'Mona Verde'}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8">
              {loading ? (
                        ...
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-[#4E5A48]/80">
                              {policyData.contactAddress.line1}<br />
                              {policyData.contactAddress.line2}<br />
                              {policyData.contactAddress.line3}
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                <div className="max-w-4xl mx-auto text-center py-12">
                  <p className="text-[#4E5A48]/60">No privacy policy available.</p>
                </div>
              )}
            </div>
